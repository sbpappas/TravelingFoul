...
if __name__ == '__main__':
    ...
    if choice == '2':
        ...
        cursor.execute('''INSERT INTO userpref (city) VALUES (%s)''', (dest_city,)) #the comma is required you must provide a tuple...
        ...
        for _ in daterange:
            ...
            dateinfo = scrapeweather(flightdate,orig_city, dest_city)
            weatherinsert(dest_city, dateinfo, flightdate)
            flightinfo = scrapeflight(flightdate,orig_city, dest_city)
            flightinsert(dest_city, orig_city, flightinfo )
        cursor.execute("SELECT * from weather WHERE (description = 'Clear sky' or description = 'Few Clouds') and dest = (%s) and date = (%s);",(dest_city,enddate))
        ...
        if goodday == None:
            ...
            date = str(goodday[5])
            print(date)
            cursor.execute("SELECT * from flights WHERE arrive_time = (%s) and dest = (%s) and orig = (%s);",(date,dest_city,orig_city))
